...
const isValidEmail = (email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);//wheather email @ or not
const Signup = (onSignupSuccess = null) => {
  ...
  //Email validation function also validation name email and password
  const validate = () => {
    const e = {};
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      if (!resp.ok) {
        setSubmitError(data?.message || "Registeration failed");
        return;
      }
      if (data?.token) {
        try {
